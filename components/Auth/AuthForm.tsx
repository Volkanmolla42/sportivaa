  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      if (mode === 'login') {
        ...
        const { user } = data;
        if (user) {
          onAuthSuccess && onAuthSuccess(user.id);
          router.push('/profile');
        }
      }
    } catch (err: any) {
      setHasError(err.message || 'Bir hata oluştu.');
    } finally {
      setIsLoading(false);
    }
  }
    ...
    >
      ...
      <div className="text-center text-sm mt-2">
        {mode === 'login' ? (
          <span>
            Hesabın yok mu?{' '}
            <a href="/auth/register" className="text-blue-600 dark:text-blue-400 underline">Kayıt Ol</a>
          </span>
        ) : (
          <span>
            Zaten hesabın var mı?{' '}
            <a href="/auth/login" className="text-blue-600 dark:text-blue-400 underline">Giriş Yap</a>
          </span>
        )}
      </div>
    </form>
